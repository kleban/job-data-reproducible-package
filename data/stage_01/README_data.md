# data/stage_01 — Stage 1 Outputs

## What this stage produces

Stage 1 reads daily job vacancy JSON files from `data/input/`, deduplicates records, cleans text (removes dates, salaries, and emoji), detects the language of each vacancy's title and description, and saves one cleaned pickle file per input day.
...
data/stage_01/
├── intermediate/
│   ├── process.pkl                  ← Stage 1 process tracker
│   ├── unic_id_db.pkl               ← Global unique vacancy ID database
│   └── id_region_click/
│       └── ua-YYYY-MM-DD.pkl        ← Per-file id / region / clicks snapshots
└── processed/
    └── output/
        └── ua-YYYY-MM-DD.pkl        ← Cleaned, deduplicated daily output files
```
...
| `output_path` | Full path to the cleaned output pickle |
| `extract_status` | Processing status: `complete` or blank |

### `intermediate/unic_id_db.pkl`
...
### `intermediate/id_region_click/ua-YYYY-MM-DD.pkl`
...
This repository includes one demo input day (`ua-2024-01-01.json`, 100 synthetic rows in `data/input/`). Running Stage 1 on the demo file produces:

- `intermediate/id_region_click/ua-2024-01-01.pkl`
- `intermediate/process.pkl` (updated)
- `intermediate/unic_id_db.pkl` (updated)
- `processed/output/ua-2024-01-01.pkl`
